...
class MoviesForm extends React.Component {
  ...
  handleSave = () => {
    ...
    const { id, name, genre, directorId, watched } = selectedValue;
    id ?
    updateMovie({id, name, genre, directorId, watched: Boolean(watched) }) :
    addMovie({ name, genre,  directorId, watched: Boolean(watched) });
    onClose();
  };

  render() {
    const { data = {}, classes, open, handleChange, handleSelectChange, handleCheckboxChange, selectedValue = {} } = this.props;
    const { name, genre, directorId, watched } = selectedValue;
